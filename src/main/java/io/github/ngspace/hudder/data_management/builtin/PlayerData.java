...
public class PlayerData extends HudderBuiltInVariables {
	...
	private static void registerPlayerStatusVariables() {
		...
		// Air bubbles
		register(k -> getCurrentAirSupplyBubble(Math.clamp(ins.player.getAirSupply(), 0, ins.player.getMaxAirSupply()),ins.player.getMaxAirSupply(),0), NUMBER, "airbubbles");

		register(k -> getCurrentAirSupplyBubble(ins.player.getMaxAirSupply(),ins.player.getMaxAirSupply(),0), NUMBER, "maxairbubbles");
	}
	...
	private static void registerPlayerStateVariables() {
		...
		register(k -> ins.gameMode.getPlayerMode() == GameType.SURVIVAL,
		        BOOLEAN, "issurvival", "is_survival");

		register(k -> ins.gameMode.getPlayerMode() == GameType.CREATIVE,
		        BOOLEAN, "iscreative", "is_creative");

		register(k -> ins.gameMode.getPlayerMode() == GameType.ADVENTURE,
		        BOOLEAN, "isadventure", "is_adventure");

		register(k -> ins.gameMode.getPlayerMode() == GameType.SPECTATOR,
		        BOOLEAN, "isspectator", "is_spectator");
	}
	...
	private static void registerMountVariables() {
		...
		register(k -> (ins.player.getVehicle() instanceof LivingEntity entity)
		        ? entity.getAttribute(Attributes.JUMP_STRENGTH).getBaseValue()
		        : null,
		    NUMBER, "mount_jump_strength");

		register(k -> (ins.player.getVehicle() instanceof AbstractHorse)
		        ? ins.player.getJumpRidingScale()
		        : null,
		    NUMBER, "mount_jump_scale");
		...
		register(k -> {
		    ...
		    return (v == null)
		            ? null
		            : v.getType().builtInRegistryHolder().key().identifier().toString();
		}, STRING, "mount_type");

		register(k -> (ins.player.getVehicle() instanceof AbstractHorse horse)
		        ? horse.getBodyArmorItem().getItem().toString()
		        : null,
		    STRING, "mount_armor_type");

		register(k -> {
		    var v = ins.player.getVehicle();
		    return (v == null || v.getCustomName() == null)
		            ? null
		            : v.getCustomName().getString();
		}, STRING, "mount_name");
	...
	private static void registerLookingAtVariables() {
		...
		register(k -> {
		    ...
		    return hit == null ? null
		            : BuiltInRegistries.BLOCK.getKey(
		                    ins.level.getBlockState(hit.getBlockPos()).getBlock()
		              ).toString();
		...
		register(k -> {
		    ...
		    return hit == null ? null
		            : BuiltInRegistries.FLUID.getKey(
		                    ins.level.getFluidState(hit.getBlockPos()).getType()
		              ).toString();
		...
		register(k -> ins.crosshairPickEntity == null
		        ? null
		        : BuiltInRegistries.ENTITY_TYPE
		              .getKey(ins.crosshairPickEntity.getType())
		              .toString(),
